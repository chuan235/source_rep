 ...
 * JCL的适配器,检测是否存在 Log4j 2.x 或者 SLF4J,否则将使用 JUL
...
final class LogAdapter {
	...
	static {
		// 首先判断是否存在 log4j2，加载org.slf4j.spi.LocationAwareLogger类
		if (isPresent(LOG4J_SPI)) {
			if (isPresent(LOG4J_SLF4J_PROVIDER) && isPresent(SLF4J_SPI)) {
				/**
				 * log4j-to-slf4j bridge -> we'll rather go with the SLF4J SPI;
				 * however, we still prefer Log4j over the plain SLF4J API since the latter does not have location awareness support.
				 * 是否存在 log4j-to-slf4j 的桥接器
				 * 加载桥接器
				 */
				logApi = LogApi.SLF4J_LAL;
			}else {
				// 不存在桥接器，加载Log4j2 Use Log4j 2.x directly, including location awareness support
				logApi = LogApi.LOG4J;
			}
		}else if (isPresent(SLF4J_SPI)) {
			// Full SLF4J SPI including location awareness support
			// 初始化完整的SLF4J SPI，支持位置感知
			logApi = LogApi.SLF4J_LAL;
		}else if (isPresent(SLF4J_API)) {
			// 纯粹使用SLF4J，没有位置感知  Minimal SLF4J API without location awareness support
			logApi = LogApi.SLF4J;
		}else {
			// 默认加载JUL  java.util.logging as default
			logApi = LogApi.JUL;
		}
	}
