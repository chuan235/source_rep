...
public class DispatcherServlet extends FrameworkServlet {
	 ...
	 * 是获取所有的还是获取beanName=handlerMapping 的bean
	 */
	...
	private void initMultipartResolver(ApplicationContext context) {
		try {
			// 从容器中获取 beanName=multipartResolver 的文件上传解析器，如果有这个组件就直接返回了
	 ...
	 * 这个方法就是初始化springMVC的所有HandllerMappings , 它首先回去容器中寻找所有的 HandlerMapping 类型的对象
	 * 当然如果我们没有手动的向spring容器中添加 这个类型的组件，那么这里是找不到的
	 * 如果在容器中找到了(存在) HandlerMapping 类型的对象，就会直接把找到的对象排序后返回
	 * 如果在容器中没有找到对应的对象，那么就会默认向容器中注册 DispatcherServlet.properties 文件中的对应的类
	 */
	private void initHandlerMappings(ApplicationContext context) {
		...
		if (this.detectAllHandlerMappings) {
			...
			try {
				// 根据beanName => handlerMappings 去获取bean
		...
		// 如果容器中不存在handlerMappings，这里就回去注册默认的handlerMappings
		// 这里的 handlerMappings 就是DispatcherServlet.properties中配置的handlerMappings
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// 				S				  => RequestMappingHandlerAdapter#handleInternal
	 ...
	 * 默认存在的handlerMapping
	 * 	BeanNameUrlHandlerMapping		beanName对应请求的uri implements Controller 这种controller会用到这个handlerMapping
	 * 	RequestMappingHandlerMapping	@RequestMapping注解对应请求的uri
	 */
	@Nullable
	...
	/**
	 * Render the given ModelAndView. 渲染ModelAndView中的视图
	 * <p>This is the last stage in handling a request. It may involve resolving the view by name.
	 * @param mv the ModelAndView to render 需要渲染的mv对象
	 * @param request current HTTP servlet request 当前请求
	 * @param response current HTTP servlet response 当前请求的响应
	 * @throws ServletException if view is missing or cannot be resolved 如果视图不能被解析或者找不到视图，会抛出 ServletException异常
	 * @throws Exception if there's a problem rendering the view 如果解析视图过程中出现问题，抛出Exception
	 */
	protected void render(ModelAndView mv, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			// AbstractView#render
			// 渲染视图  map<Object,Object> request response
	...
	protected View resolveViewName(String viewName, @Nullable Map<String, Object> model,
		...
		if (this.viewResolvers != null) {
			for (ViewResolver viewResolver : this.viewResolvers) {
				// 使用视图解析器来解析视图
				// InternalResourceViewResolver  =调用父类=> AbstractCachingViewResolver#resolveViewName
