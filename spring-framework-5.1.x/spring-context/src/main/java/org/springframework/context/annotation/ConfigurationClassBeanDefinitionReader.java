...
class ConfigurationClassBeanDefinitionReader {
	 ...
	 * with the registry based on its contents. 注册
	 */
	 ...
	 * 注册配置类内部的所有@Bean方法，配置类会在扫描出来的时候就注册到了bd Map中
	 */
	private void loadBeanDefinitionsForConfigurationClass(
		...
		// 如果这个 ConfigurationClass 是被Import的那么将会进入这个if
		// 最终会将这个对象存放入 bdMap
		...
		// 处理importSource xml Groovy
		loadBeanDefinitionsFromImportedResources(configClass.getImportedResources());
		// 处理 importBeanDefinitionRegistrars Map中的类，这是import进来的 BeanDefinitionRegistrar
		// 通过遍历注册到 bdMap 中
	...
	private void loadBeanDefinitionsForBeanMethod(BeanMethod beanMethod) {
		...
		if (metadata.isStatic()) {
			...
			// 设置了唯一的工厂方法，创建时回调用这个方法，为不是直接去new
		...
		// 处理通用的注解 lazy DependsOn Primary Role Description
		...
		// Replace the original bean definition with the target one, if necessary 转换接口
