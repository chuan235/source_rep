...
class LoadTimeWeaverBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
	...
	protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder) {
		...
		if (isAspectJWeavingEnabled(element.getAttribute(ASPECTJ_WEAVING_ATTRIBUTE), parserContext)) {
			// 向容器中注册了一个组件
			// ASPECTJ_WEAVING_ENABLER_BEAN_NAME  AspectJWeavingEnabler
	...
	/**
	 * <context:load-time-weaver  aspectj-weaving="autodetect" /> 取值有on、off、autodetect
	 * @param value
	 * @param parserContext
	 * @return
	 */
	protected boolean isAspectJWeavingEnabled(String value, ParserContext parserContext) {
		if ("on".equals(value)) {
			...
			// Determine default...
			// 默认的会加载META-INF/aop.xml这个文件，来决定能否使用aspectJ
