...
public interface Lifecycle {
	 ...
	 * 实现SmartLifecycle 和它的 stop(Runnable) 方法可以完成异步停止
	 ...
	 * <p>In the case of a container, this will return {@code true} only if <i>all</i>components that apply are currently running.
	 * 检查该组件是否正在运行
	 * 只有这个方法返回false时，start方法才会被执行
	 * 只有这个方法返回true时，stop(Runnable callback)或stop()方法才会被执行
