...
public interface SmartLifecycle extends Lifecycle, Phased {
	 ...
	 * 表示是否自动回调 Lifecycle 接口中的方法
	 * 返回false表示和一般的 Lifecycle 接口实现类无差别
	 */
	 ...
	 * @see #stop()
	 * @see #getPhase()
	 * 接受一个回调方法，任何一个实现了这个方法的类都会在关闭的stop方法调用完成之后，开始调用回调对象的run方法
	 * 必要时支持异步关闭，因为LifecycleProcessor接口的默认实现DefaultLifecycleProcessor会等待每个阶段中的对象组的超时值来调用回调。
	 * 默认的每个阶段超时时间是30秒。
	 * 通过在上下文中定义一个名为lifecycleProcessor的bean，可以覆盖默认的lifecycle processor实例。
	 * 如果想要修改这个值，只需增加以下配置
