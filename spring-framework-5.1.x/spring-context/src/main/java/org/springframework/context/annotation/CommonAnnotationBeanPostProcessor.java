...
public class CommonAnnotationBeanPostProcessor extends InitDestroyAnnotationBeanPostProcessor
	...
	// 第一次访问：CommonAnnotationBeanPostProcessor.postProcessMergedBeanDefinition  ->  BeanPostProcessor的第三次回调 （解析注解 缓存注解）
	// 第二次访问：CommonAnnotationBeanPostProcessor#postProcessProperties -> BeanPostProcessor的第六次回调（直接从缓存中拿）
	private InjectionMetadata findResourceMetadata(String beanName, final Class<?> clazz, @Nullable PropertyValues pvs) {
		// Fall back to class name as cache key, for backwards compatibility with custom callers.
		String cacheKey = (StringUtils.hasLength(beanName) ? beanName : clazz.getName());
		// Quick check on the concurrent map first, with minimal locking. 检查缓存中是否存在
		/**
		 * InjectionMetadata   injectionMetadataCache(存储了已经完成注入的bean对象,并且已经将依赖缓存起来了)
		 * 	targetClass 当前需要注入的class  userTest2
		 * 	injectedElements CommonAnnotationBeanPostProcessor$ResourceElement 类型（是一个List集合，需要注入的属性）
		 * 	checkedElements  检查通过的注入类型
		 */
		InjectionMetadata metadata = this.injectionMetadataCache.get(cacheKey);
		// 这个条件 (metadata == null || metadata.targetClass != clazz)
