...
public class InitDestroyAnnotationBeanPostProcessor
	...
	private LifecycleMetadata findLifecycleMetadata(Class<?> clazz) {
		if (this.lifecycleMetadataCache == null) {
			// Happens after deserialization, during destruction... 返回缓存中存在的  初始化缓存map对象
			return buildLifecycleMetadata(clazz);
		}
		// Quick check on the concurrent map first, with minimal lo	cking. 从Map获取生命周期回调的元数据
		// 包括回调的类 初始方法集合  销毁方法集合
		LifecycleMetadata metadata = this.lifecycleMetadataCache.get(clazz);
		if (metadata == null) {
			synchronized (this.lifecycleMetadataCache) {
				metadata = this.lifecycleMetadataCache.get(clazz);
				if (metadata == null) {
					// 再次检查缓存中没有就创建，并放入map缓存中
	...
	/**
	 * 从一个Class类中获取生命会回调元数据 => LifecycleMetadata
	 * 利用反射获取类中的所有方法，检查注解 @PostConstruct/@PreDestroy 不会检查方法名称
	 * @param clazz
	 * @return
	 */
	private LifecycleMetadata buildLifecycleMetadata(final Class<?> clazz) {
		...
		do {
			...
			ReflectionUtils.doWithLocalMethods(targetClass, method -> {
				// 这里的initAnnotationType和destroyAnnotationType在CommonAnnotationBeanPostProcessor创建的时候设置了 为注解形式
	...
	private static class LifecycleElement {
		...
		public LifecycleElement(Method method) {
			...
			this.method = method;
			// private : method.getDeclaringClass()).getName() + '.' + method.getName();
			// 非private： method.getName();
