...
public class CustomAutowireConfigurer implements BeanFactoryPostProcessor, BeanClassLoaderAware, Ordered {
	...
	/**
	 * Register custom qualifier annotation types to be considered when autowiring beans. Each element of the provided set may
	 ...
	 * 注册自动装配Bean时要考虑的自定义限定符注释类型。
	 * 提供的集合中的每个元素都可以是自定义批注的全限定类名的Class实例或String表示。
	 * 请注意，任何本身使用Spring批注进行批注的批注
	 */
