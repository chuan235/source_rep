...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			// step2:获取resource的inputStream --转化为--> inputSource
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
			// 解析xml：jdk中dom解析、dom4j、SAX
		try {
			// 真正解析xml的方法，使用的技术是SAX
			// step3:SAX解析xml
			Document doc = doLoadDocument(inputSource, resource);
			// step4:将document中对应的对象注册到bdMap中
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		// see DefaultDocumentLoader#loadDocument
		return this.documentLoader.loadDocument(
				inputSource,
				// 向SAX驱动器注册一个EntityResolver实例，保存了xsd文件本次存放的路径
				// 在spring各个包下的resources/META-INF/spring.schemas中定义了本地xsd文件存放的路径
				// spring-framework-5.1.x\spring-beans\src\main\resources\org\springframework\beans\factory\xml\spring-beans.dtd
				// spring-framework-5.1.x\spring-beans\src\main\resources\META-INF\spring.schemas
				getEntityResolver(),
				// SimpleSaxErrorHandler对象
				this.errorHandler,
				// XML验证方式的读取
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 创建一个bd的文档读取器 读取document中的bean
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		// 统计已经存在的bd个数
		int countBefore = getRegistry().getBeanDefinitionCount();
		// 注册bd  DefaultBeanDefinitionDocumentReader#registerBeanDefinitions
		// XmlReaderContext 中就包含了schema 对应的 xxxNamespaceHandler
