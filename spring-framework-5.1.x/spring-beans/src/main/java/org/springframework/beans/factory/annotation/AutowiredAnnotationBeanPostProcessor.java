...
public class AutowiredAnnotationBeanPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
	...
	public AutowiredAnnotationBeanPostProcessor() {
		// 设置当前的processor需要解析的注解  @Autowired @Value  @Inject
	...
	// 和CommonAnnotationBeanPostProcessor一样  分两次调用  第一次解析  第二次从缓存中获取
	...
	private InjectionMetadata buildAutowiringMetadata(final Class<?> clazz) {
		...
		do {
			...
			// 拿到了所有的autowiredAnnotationTypes（@Autowired @Value @Inject）字段
			ReflectionUtils.doWithLocalFields(targetClass, field -> {
				AnnotationAttributes ann = findAutowiredAnnotation(field);
				if (ann != null) {
					// 如果方法或者属性是静态的这里直接返回了 如果需要注入静态变量 需要使用set方法或者实现ApplicationContextAware
	...
	private AnnotationAttributes findAutowiredAnnotation(AccessibleObject ao) {
		if (ao.getAnnotations().length > 0) {
			// autowiring annotations have to be local 获取构造方法上的注解
