...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
		...
		// 遍历所有的 BeanPostProcessor 执行postProcessBeforeInitialization方法
		// InitDestroyAnnotationBeanPostProcessor 执行@PostConstruct注解的方法
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// Make sure bean class is actually resolved at this point, and clone the bean definition in case of a dynamically resolved Class
		...
		// Prepare method overrides. 处理lookup-method and replace-method
		...
		try {
			...
			// 是否需要spring来管理这个类 可以通过实现 InstantiationAwareBeanPostProcessor 接口
			// 这里就会返回这个接口中 postProcessBeforeInstantiation 方法返回的对象，spring中默认返回的是null
			// 表示spring会去维护这个类中的一些依赖关系，如果我们不需要spring来维护这些但是又希望将spring将这个类放入容器中
			// 就可以通过这个方法，将我们需要放入spring的对象，返回，这里会直接返回这个对象，结束这个bean的创建
			// 第一次调用 BeanPostProcessor InstantiationAwareBeanPostProcessor 的 postProcessBeforeInstantiation 方法
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		// Instantiate the bean. 包装真实的bean对象
		...
		// 第三次后置处理器的回调  缓存声明周期函数注解，并不会执行
		// 最先调用 MergedBeanDefinitionPostProcessor 的 postProcessMergedBeanDefinition 方法
	...
	protected Object applyBeanPostProcessorsBeforeInstantiation(Class<?> beanClass, String beanName) {
		for (BeanPostProcessor bp : getBeanPostProcessors()) {
			if (bp instanceof InstantiationAwareBeanPostProcessor) {
				// CommonAnnotationBeanPostProcessor
				// AutowiredAnnotationBeanPostProcessor
				// configurationClassPostProcessor$ImportAwareBeanPostProcessor 空实现postProcessBeforeInstantiation方法
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		// Make sure bean class is actually resolved at this point. 确保bd中的class已经被加载进来了
	...
	protected Constructor<?>[] determineConstructorsFromBeanPostProcessors(@Nullable Class<?> beanClass, String beanName)
			throws BeansException {
		// 执行 AutowiredAnnotationBeanPostProcessor.determineCandidateConstructors
	...
	/**
	 * 构造函数自动装配(根据类型去装配)，需要借助构造函数的解析器组件 => ConstructorResolver
	 * "autowire constructor" (with constructor arguments by type) behavior.
	 * Also applied if explicit constructor argument values are specified,
	 * matching all remaining arguments with beans from the bean factory.
	 * <p>This corresponds to constructor injection:
	 * In this mode, a Spring bean factory is able to host components that expect constructor-based
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// 第五次回调后置处理器，是否需要进行属性装配
		// InstantiationAwareBeanPostProcessor 的 postProcessAfterInstantiation 方法
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
