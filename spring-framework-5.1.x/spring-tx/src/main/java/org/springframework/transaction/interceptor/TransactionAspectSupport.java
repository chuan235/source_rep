...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// 获取事务注解信息，如果为null说明不需要事务
		// If the transaction attribute is null, the method is non-transactional.
		TransactionAttributeSource tas = getTransactionAttributeSource();
		final TransactionAttribute txAttr = (tas != null ? tas.getTransactionAttribute(method, targetClass) : null);
		// 确定当前事务要使用的事务管理器 直接找出默认的dataSourceTransactionManager
		final PlatformTransactionManager tm = determineTransactionManager(txAttr);
		// txAttr.getDescriptor() -> 指向匹配的方法（top.gmfcj.service.impl.UserServiceImpl1.query）
		final String joinpointIdentification = methodIdentification(method, targetClass, txAttr);

		if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
			// Standard transaction demarcation with getTransaction and commit/rollback calls. 标准的transaction开启
			TransactionInfo txInfo = createTransactionIfNecessary(tm, txAttr, joinpointIdentification);

			Object retVal;
			try {
				// 执行 lambda 表达式  懒加载  ReflectiveMethodInvocation.proceed
				retVal = invocation.proceedWithInvocation();
			}catch (Throwable ex) {
				// target invocation exception 目标方法执行出现了异常，进入异常处理流程
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		// 如果没有名称，就使用方法名作为名称 top...xx..query()
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			if (txInfo.transactionAttribute != null && txInfo.transactionAttribute.rollbackOn(ex)) {
				...
				// We don't roll back on this exception. 这个异常不需要回滚，则进行提交
