...
public class RegistryDirectory<T> extends AbstractDirectory<T> implements NotifyListener {
    ...
    public synchronized void notify(List<URL> urls) {
        List<URL> categoryUrls = urls.stream()
                // 拿到url中的 category 默认是 providers，判断是不是 routers/providers/configurators/dynamicconfigurators/appdynamicconfigurators
                .filter(this::isValidCategory)
                // 获取 compatible_config 参数的值，是不是为空，是 empty 可以继续向下走
                .filter(this::isNotCompatibleFor26x)
                .collect(Collectors.toList());
        for (URL url : urls) {
            if(url.getProtocol().equals("http")){
                System.out.println("已找到服务地址 url ="+url.toFullString());
            }
        }
        /**
         * TODO Try to refactor the processing of these three type of urls using Collectors.groupBy()?
         * isConfigurator 方法判断是 OVERRIDE 协议或者 configurators 配置时返回 true
         */
        this.configurators = Configurator.toConfigurators(classifyUrls(categoryUrls, UrlUtils::isConfigurator))
                .orElse(configurators);
        // 是 route 协议或者 routers 配置时返回 true
