...
public class ServiceBean<T> extends ServiceConfig<T> implements InitializingBean, DisposableBean,
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        System.out.println("ServiceBean监听到容器刷新事件,准备导出服务....");
        // 是否已经导出 && 是否取消导出
    ...
    /**
     *  这里就是初始化，一个service，到底属于哪个应用，哪个provider，那么modual，
     *  我们在配置文件中并没有明确的指定，所以需要在这里去进行初始化，这里并没有配置直接的覆盖
     * 这个方法会在这个对象初始化之后执行
     * 这里的各种config对象都会在AbstractApplicationContext#refresh方法中被加入bdMap中并在这里之前实例化完成
     */
    @Override
    @SuppressWarnings({"unchecked", "deprecation"})
    public void afterPropertiesSet() throws Exception {
        System.out.println("ServiceBean开始组装配置....");
        ...
        // 找出当前这个service所对应的Application,并set  从spring中拿ApplicationConfig对象
        ...
        // 找出当前这个service所对应的Module,并set ModuleConfig
        if (getModule() == null
