...
public abstract class AbstractConfig implements Serializable {
                ...
                if (ClassHelper.isGetter(method)) {
                    ...
                    if (value != null && str.length() > 0) {
                        ...
                        if(parameters.size()> 10){
                            System.out.println("parameters参数="+parameters);
                        }
    ...
    private static String calculatePropertyFromGetter(String name) {
        ...
        // getSimpleProviderConfig -> simple.provider.config
        ...
        for (Method method : methods) {
            try {
                ...
                if (isMetaMethod(method)) {
                    ...
                    String key;
                    Parameter parameter = method.getAnnotation(Parameter.class);
                    // 拿Parameter注解的key 和 useKeyAsProperty 来替换原本的属性名称
                    if (parameter != null && parameter.key().length() > 0 && parameter.useKeyAsProperty()) {
                        key = parameter.key();
                    } else {
                        // 如果不存在Parameter注解，或者Parameter注解上的key不使用，则将属性名作为key
                        key = prop;
                    }
...
//                            String pre = (prefix != null && prefix.length() > 0 ? prefix + "." : "");
    ...
    /**
     * Currently, only support overriding of properties explicitly defined in Config class, doesn't support
