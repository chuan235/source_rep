...
public class ClientCnxn {
    ...
    public ClientCnxn(String chrootPath, HostProvider hostProvider, int sessionTimeout, ZooKeeper zooKeeper,
        ...
        // 连接超时时间 sessionTimeout/hostSize
        // 创建客户端的时候可以输入多个服务器地址，这里就是输入的服务器地址的数量
        connectTimeout = sessionTimeout / hostProvider.size();
        // 读取超时时间
        readTimeout = sessionTimeout * 2 / 3;
        readOnly = canBeReadOnly;
        // 创建了两个线程  SendThread  EventThread
    ...
    class EventThread extends ZooKeeperThread {
        ...
        public void queueEvent(WatchedEvent event) {
            ...
            /**
             * 返回具体的watcher集合
             * @see ZKWatchManager#materialize
             */
        ...
        private void processEvent(Object event) {
            try {
                ...
                if (event instanceof WatcherSetEventPair) {
                    ...
                    // 循环 pair 中的所有 watcher
                    for (Watcher watcher : pair.watchers) {
                        try {
                            // 执行具体watcher的process方法
    ...
    private void finishPacket(Packet p) {
        ...
        if (p.cb == null) {
            synchronized (p) {
                // 当前的packet p 标识为已完成
                p.finished = true;
                // 唤醒在SendThread 的 run方法中阻塞的packet
                p.notifyAll();
            }
        } else {
    ...
    class SendThread extends ZooKeeperThread {
        ...
        void readResponse(ByteBuffer incomingBuffer) throws IOException {
            ...
            if (replyHdr.getXid() == -4) {
                // -4 is the xid for AuthPacket 认证packet
            ...
            if (replyHdr.getXid() == -1) {
                // -1 means notification watcher响应
                if (LOG.isDebugEnabled()) {
            ...
            // If SASL authentication is currently in progress, construct and send a response packet immediately, rather than queuing a response as with other packets.
            // 验证响应
        ...
        void primeConnection() throws IOException {
            ...
            // outgoingQueue 中放的是 一个一个的 Packet
                ...
                if (!disableAutoWatchReset) {
                    ...
                    if (!dataWatches.isEmpty() || !existWatches.isEmpty() || !childWatches.isEmpty()) {
                        ...
                        while (dataWatchesIter.hasNext() || existWatchesIter.hasNext() || childWatchesIter.hasNext()) {
                            ...
                            h.setXid(-8);
                            // requestHeader时 watcher 类型的header
            ...
            while (state.isAlive()) {
                try {
                    ...
                    if (state.isConnected()) {
                        ...
                        //1000是防止丢失竞争条件来发送第二个ping，确保在readTimeout较小时不要发送太多的ping
        ...
        void onConnected(int _negotiatedSessionTimeout, long _sessionId, byte[] _sessionPasswd, boolean isRO) throws IOException {
            // negotiatedSessionTimeout=sessionTimeout
            negotiatedSessionTimeout = _negotiatedSessionTimeout;
            if (negotiatedSessionTimeout <= 0) {
                // session已经超时
                state = States.CLOSED;
                // 发布一个 None型的session过期状态 事件
            ...
            if (!readOnly && isRO) {
                // 具有读写操作的客户端不能连接只读的服务器
                LOG.error("Read/write client got connected to read-only server");
            }
            // 通过session超时时间计算出读取超时时间
            ...
            // 发布连接None型的 ConnectedReadOnly/SyncConnected 状态事件
    ...
    public ReplyHeader submitRequest(RequestHeader h, Record request,
        ...
        synchronized (packet) {
            // 1、主动唤醒，不存在callback
            // 2、finished=true 退出循环，存在callback
    ...
    Packet queuePacket(RequestHeader h, ReplyHeader r, Record request,
        ...
        synchronized (outgoingQueue) {
            ...
            if (!state.isAlive() || closing) {
                ...
                // 最后将这个请求的packet加入到 outgoingQueue 队列中
