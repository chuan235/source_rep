...
public class LearnerHandler extends ZooKeeperThread {
    ...
    private void sendPackets() throws InterruptedException {
        ...
        while (true) {
            try {
                ...
                if (p == null) {
                    // 如果没有取出数据，会刷新输出管道，使用take方法去拿，拿不到会阻塞在这里
    ...
    public void run() {
        try {
            ...
            /* the default to send to the follower 默认向follower发送快照 */
            ...
            try {
                ...
                if (peerLastZxid == leader.zk.getZKDatabase().getDataTreeLastProcessedZxid()) {
                    // Follower is already sync with us, send empty diff 与leader的zxid相同，表示已经同步完成
                    ...
                    if ((maxCommittedLog >= peerLastZxid) && (minCommittedLog <= peerLastZxid)) {
                        ...
                        // Before sending the first packet, we have to tell the learner whether to expect a trunc or a diff
                        ...
                        for (Proposal propose : proposals) {
                            ...
                            if (propose.packet.getZxid() <= peerLastZxid) {
                                // 确定同步的范围[propose.packet.getZxid(), peerLastZxid]
                                prevProposalZxid = propose.packet.getZxid();
                                continue;
                            } else {
                                // If we are sending the first packet, figure out whether to trunc in case the follower has some proposals that the leader doesn't
                                // 检查第一次发送数据包是否需要TRUNC，防止follower的提议进入这里
                                if (firstPacket) {
                                    firstPacket = false;
                                ...
                                // 将这个提议扔到队列中，这个提议也需要传输给follower
                                queuePacket(propose.packet);
                                // 构建commit，直接提交到follower上 => COMMIT
                        ...
                        // 如果follower上存在多余的zxid，将 maxCommittedLog 之后的记录都删除
                        packetToSend = Leader.TRUNC;
                        zxidToSend = maxCommittedLog;
                        updates = zxidToSend;
                    } else {
            ...
            if (packetToSend == Leader.SNAP) {
                // 如果是快照类型的，需要获取最后一次打快照的zxid
                zxidToSend = leader.zk.getZKDatabase().getDataTreeLastProcessedZxid();
            }
            // 发送了一个null的数据包，但是给了一个zxid的范围，follower可以根据这个返回清理旧数据
            ...
            if (packetToSend == Leader.SNAP) {
                ...
                // Dump data to peer  将快照序列化之后发送给learner
                leader.zk.getZKDatabase().serializeSnapshot(oa);
                oa.writeString("BenWasHere", "signature");
            }
            bufferedOutput.flush();

            // Start sending packets 发送额外的zxid和NEWLEADER
            new Thread() {
                public void run() {
                    Thread.currentThread().setName(
                            "Sender-" + sock.getRemoteSocketAddress());
                    try {
                        // 发送 queuedPackets 阻塞队列中的packet
                        sendPackets();
