...
public class CommitProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    /**
     * sync的操作类型或者其他不在处理范围之内的request会放入这个集合中
     */
    ...
    public void run() {
        try {
            ...
            while (!finished) {
                // 如果这个toProcess中存在request，直接调用nextProcessor优先处理toprocess中的请求
                ...
                synchronized (this) {
                    if ((queuedRequests.size() == 0 || nextPending != null)
                        ...
                        // 如果 committedRequests 没有对应的request，那么会阻塞在这里等待commit，commit中一旦添加了request，会唤醒这里
                        // 先完成日志的记录
                        wait();
                        continue;
                    }
                    // First check and see if the commit came in for the pending
                    // 如果存在nextPending和committedRequest，将request加入toProcess集合中
                    // request
