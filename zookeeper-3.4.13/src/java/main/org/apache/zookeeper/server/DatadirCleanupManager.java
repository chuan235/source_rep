 ...
 * logs by scheduling the auto purge task with the specified 'autopurge.purgeInterval'.
 * It keeps the most recent 'autopurge.snapRetainCount' number of snapshots and corresponding transaction logs.
 */
public class DatadirCleanupManager {
    ...
    public void start() {
        ...
        // purgeInterval默认为0，表示不开启自动清理功能
        if (purgeInterval <= 0) {
            LOG.info("Purge task is not scheduled.");
            return;
        }
        // 如果配置了autopurge.snapRetainCount和autopurge.purgeInterval
        // autopurge.purgeInterval=2指定清理数据的频率，单位是小时，需要一个比0大的整数
        // autopurge.snapRetainCount=3这个参数指定了需要保留的快照文件数目，默认是保留3个
        // 这里就会开启一个后台的守护线程来进行数据的清理
        // new了一个TimerThread类
        timer = new Timer("PurgeTask", true);
        TimerTask task = new PurgeTask(dataLogDir, snapDir, snapRetainCount);

        // 将task任务交给TimerThread类去执行  执行这个task的run方法
        // scheduleAtFixedRate 就是计划task执行的时机
        // 服务启动的时候会执行一次，之后在执行成功后的purgeInterval小时后会再次执行
        timer.scheduleAtFixedRate(task, 0, TimeUnit.HOURS.toMillis(purgeInterval));
        // 修改 purgeTaskStatus 为started已启动
