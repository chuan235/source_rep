...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    public void loadData() throws IOException, InterruptedException {
        /*
         * When a new leader starts executing Leader#lead, it invokes this method. The database,
         * however, has been initialized before running leader election so that the server could pick its zxid for its initial vote.
    ...
    public void submitRequest(Request si) {
        if (firstProcessor == null) {
            ...
            synchronized (this) {
                try {
                    // Since all requests are passed to the request processor it should wait for setting up the request processor chain.
                    // The state will be updated to RUNNING after the setup.
    ...
    public void processPacket(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        if (h.getType() == OpCode.auth) {
            ...
            if (authReturn != KeeperException.Code.OK) {
                ...
                // 验证失败 send a response... Failed：-115
