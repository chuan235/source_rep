...
public class NIOServerCnxn extends ServerCnxn {
    ...
    /**
     * NIO中的通道，相比于传统的单通道的IO模型，NIO中的Channel既可以用来读操作，也可以用来写操作
     * 传统的I/O模型中，Stream流都是单向的，比如InputStream只能读，OutputStream只能写
     */
    ...
    /**
     * Buffer类：在NIO中所有数据的读和写都离不开Buffer，读取的数据只能放在buffer中，写入数据时也只是先写入到Buffer中
     */
            ...
            if (k.isWritable()) {
                ...
                if (outgoingBuffers.size() > 0) {
                    ...
                    for (ByteBuffer b : outgoingBuffers) {
                        ...
                        /*
                         * put() is going to modify the positions of both buffers,
                         * put we don't want to change the position of
                         * the source buffers (we'll do that after the send, if needed),
                         * so we save and reset the position after the copy
                         */
                        int p = b.position();
                        directBuffer.put(b);
                        b.position(p);
