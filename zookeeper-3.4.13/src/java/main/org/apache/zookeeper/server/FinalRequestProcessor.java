...
public class FinalRequestProcessor implements RequestProcessor {
    ...
    public void processRequest(Request request) {
        ...
        // 阻塞List，取出list中的ChangeRecord去更新内存中的数据  取出closeSession提交的请求
        ...
        if (request.hdr != null && request.hdr.getType() == OpCode.closeSession) {
            ...
            // this might be possible since we might just be playing diffs from the leader
            if (scxn != null && request.cnxn == null) {
                // calling this if we have the cnxn results in the client's close session response being lost -
                // we've already closed the session/socket here before we can send the closeSession in the switch block below
