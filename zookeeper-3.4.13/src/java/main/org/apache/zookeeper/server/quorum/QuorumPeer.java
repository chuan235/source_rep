...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    /**
     * LOOKING：寻找Leader状态，当服务器处于这个状态时，他会认为集群中没有Leader，进入Leader选举状态
     */
    ...
    /**
     * This is who I think the leader currently is. 当前机器认为哪一个是Leader
     */
            ...
            try {
                ...
                while (running) {
                    ...
                    if (packet.getLength() != 4) {
                        ...
                        // sate 默认为LOOKING, 这表明选举还没有完成，只要选举完成，每一个节点都会有自己的角色
                        // LEADING / FOLLOWING / OBSERVING
                        switch (getPeerState()) {
                            case LOOKING:
                                // serverId(当前投票的serverId) serverId(投给哪一个机器的serverId) zxid
    ...
    public synchronized void start() {
        // 集群启动时，开始加载本机的数据 更新epoch
        loadDataBase();
        // 开启读取数据的线程，leader和follower/observer的交互
        cnxnFactory.start();
        // 进行Leader选举，确定服务器的角色，在针对不同角色的服务器角色进行初始化
        startLeaderElection();
        super.start();
    }
    ...
    private void loadDataBase() {
        ...
        try {
            // 加载快照和datalog中的数据
            zkDb.loadDataBase();
            // load the epochs 加载epochs信息  将epoch更新为最大的届数
            long lastProcessedZxid = zkDb.getDataTree().lastProcessedZxid;
            // 根据最后一次快照中操作事务id，拿到 epoch
            long epochOfZxid = ZxidUtils.getEpochFromZxid(lastProcessedZxid);
            try {
                // 从epochFile中获取最新的epoch
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm) {
        ...
        switch (electionAlgorithm) {
            ...
            case 2:
                ...
                break;
            case 3:
                // 会为每一台机器创建一个 QuorumCnxManager，初始化每台服务器底层Leader选举的网络通信模型
    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                switch (getPeerState()) {
                    // Looing状态表示正在进行集群建立，搜索集群中的服务器，进行加载数据，leader的选取
                    case LOOKING:
                        LOG.info("LOOKING");
                        if (Boolean.getBoolean("readonlymode.enabled")) {
                            // 只读服务器的逻辑
                            LOG.info("Attempting to start ReadOnlyZooKeeperServer");
                            // 创建了一个只读的服务器但不立即启动 Create read-only server but don't start it immediately
                            final ReadOnlyZooKeeperServer roZk = new ReadOnlyZooKeeperServer(
                                    logFactory, this,new ZooKeeperServer.BasicDataTreeBuilder(),this.zkDb);

                            // Instead of starting roZk immediately, wait some grace period before we decide we're partitioned.
                            // Thread is used here because otherwise it would require changes in each of election strategy classes which is unnecessary code coupling.
                            ...
                            try {
                                 ...
                                 * 1、当集群的Leader挂了之后
                                 *  follower出现异常的位置:
                                 *      Follower#followLeader -> follower.shutdown() -> follower的状态设置为LOOKING
                                 *      -> 开启新一轮的领导者选举
                                 *      follower.shutdown();
                                 *      setFollower(null);
                                 *      setPeerState(ServerState.LOOKING);
                                 * 2、当集群中的一半的follower都挂了之后
                                 *  出现异常的位置：
                                 *      Leader#lead 中，在一个while(true)中会检测follower是否可以ping通
                                 *      如果超过一半的follower不能ping通，则会shutdown掉整个集群，并且这个方法会返回
                                 *      setLeader(null);
                                 *      setPeerState(ServerState.LOOKING);
                                 *    这里就算是重新选举也不可嫩选出leader了，因为挂了一半的follower
                                 *    投票永远不会过半，整个集群就无法使用
                                 * 3、如果新加入了一个follower
                                 *  WorkerReceiver
                                 *
                    ...
                    case FOLLOWING:
                        ...
                        try {
                            ...
                            // Follower#followLeader 如果leader挂了之后，这里会出现错误
                            // 因为这里面需要去连接Leader，如果Leader连接不上了，自然会在这里跑出异常
                            follower.followLeader();
                        } catch (Exception e) {
                            LOG.warn("Unexpected exception", e);
                        } finally {
                            // 关闭follower，就是将 zkServer/NIOServerCnxnFactory 给关闭了
                            follower.shutdown();
                            setFollower(null);
                            // 修改当前节点的状态为LOOKING,如果设置为了Looking，就会重新开启领导者的选举
                            setPeerState(ServerState.LOOKING);
                        }
                        break;
                    case LEADING:
                        ...
                        try {
                            ...
                            // Leader.lead
                            leader.lead();
                            // 如果因为超过一半的follower挂掉了，返回到这里，首先会将Leader手动置为null
                            setLeader(null);
                        } catch (Exception e) {
                            LOG.warn("Unexpected exception", e);
                        } finally {
