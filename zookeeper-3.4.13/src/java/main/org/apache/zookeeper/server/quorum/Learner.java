...
public class Learner {
    ...
    /**
     * write a packet to the leader 向leader发送packet
     *
     ...
     *
     * @param pktType
     * @return the zxid the Leader sends for synchronization purposes. 这个方法是Learner中的方法，所有的follower都会调用这个方法
     * @throws IOException
     */
    ...
    /**
     * Finally, synchronize our history with the Leader.  DIFF/SNAP/TRUNC
     *
     * @param newLeaderZxid
     * @throws IOException
     * @throws InterruptedException
    ...
    protected void syncWithLeader(long newLeaderZxid) throws IOException, InterruptedException {
        ...
        synchronized (zk) {
            if (qp.getType() == Leader.DIFF) {
                ...
                // 更新lastProcessedZxid，最后一次打快照的zxid
                zk.getZKDatabase().setlastProcessedZxid(qp.getZxid());
            } else if (qp.getType() == Leader.TRUNC) {
                //we need to truncate the log to the lastzxid of the leader 需要移除部分数据
            ...
            //If we are not going to take the snapshot be sure the transactions are not applied in memory but written out to the transaction log
            //  如果需要同步snap，这里的 writeToTxnLog = false
            /// 不需要同步snap，这里的 writeToTxnLog = true
